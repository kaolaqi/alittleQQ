...
export function setBeforeDate(date, year, month,config){
	var beforeMonthDayNumber = getMonthIsDay( getConfgMonth(config.defaultDate,-1) , config.defaultDate ),
        oneDay_week = weekOrNumber[ getWeek( getConfgDay(config.defaultDate,1) ) ];
...
export function setNowDate(date, year, month,config,template){
    var monthDayNumber = getMonthIsDay(config.defaultDate),
        oneDay_week = weekOrNumber[ getWeek( getConfgDay(config.defaultDate,1) ) ];
      //计算这个月
      var count = oneDay_week; //星期的转换计数器
      var now = config.defaultDate.getDate(); //获取当天
...
export function setAtterDate(date, year ,month,config){
 //向后补下个月的天数
  var atterMonth = getConfgMonth(config.defaultDate,1),
